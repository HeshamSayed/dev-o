    ...
    async def _request(
        ...
        try:
            ...
            if response.is_error:
                # Extract error message from response (try different keys)
                error_msg = (
                    data.get("detail") or
                    data.get("error") or
                    data.get("message") or
            ...
            raise APIError(
                status_code=0,
                message=f"Failed to connect to API at {self.api_url}. Error: {str(e)}",
            )
            ...
            raise APIError(
                status_code=0,
                message=f"API request failed: {type(e).__name__}: {str(e)}",
            )
