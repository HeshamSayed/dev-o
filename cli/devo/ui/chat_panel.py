...
class ChatPanel:
    ...
    def render_agent_start(self, event: Dict[str, Any]) -> None:
        ...
        # Simple status message without a panel
        self.console.print(f"\n[dim cyan]{agent} is working...[/dim cyan]\n")
    ...
    def render_content_chunk(self, event: Dict[str, Any]) -> None:
        ...
        # SSE format uses "response", WebSocket might use "chunk", "content", or "text"
        chunk = event.get("response") or event.get("chunk") or event.get("content") or event.get("text") or ""
    ...
    def render_stream_complete(self, event: Dict[str, Any]) -> None:
        ...
        if self.streaming_live:
            ...
            # Clear the streaming content (already displayed, no need to show again)
            self.streaming_content = ""
    ...
    def render_task_completed(self, event: Dict[str, Any]) -> None:
        ...
        # Only show summary if it's meaningful (not JSON tool calls)
        if summary and not summary.strip().startswith('{') and not summary.strip().startswith('```'):
            self.console.print(f"[dim]{summary[:200]}...[/dim]\n" if len(summary) > 200 else f"[dim]{summary}[/dim]\n")
    ...
    def render_done(self, event: Dict[str, Any]) -> None:
        """Render done event - silent, just cleanup."""
        # Silent - task_completed already showed success
        pass

    def render_connected(self, event: Dict[str, Any]) -> None:
        """Render connected event - silent, just track state."""
        # Don't display anything - the agent_start event will handle this
        pass
    ...
    def render_event(self, event: Dict[str, Any]) -> None:
        ...
        if handler:
            ...
            # Don't display anything to avoid UI clutter
            pass
